...
![Docker Compose](./../assets/docker-compose-app.svg "Docker Explained")
...
Lets first start with docker-compose.yaml :
```
version: "3.8"
...
networks: 
    ...
    backend:
        ...
        driver: bridge
```
The code for the networks block:

Two bridge networks are defined here. By default, Compose creates a bridge network and attaches all containers to that. In this project, however, proper network isolation is required. So two networks are defined, one for the front-end services and one for the back-end services.

Networks are defined in each block of the service definitions. This way the the api and db service will be attached to one network and the client service will be attached to a separate network. But the nginx service will be attached to both the networks so that it can perform as router between the front-end and back-end services.
